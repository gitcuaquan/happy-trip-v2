  ...
  <div class="min-h-screen bg-gray-50">
      ...
      <UContainer>
        <div class="flex items-center gap-3 py-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p v-if="orderDetail" class="text-xs text-muted font-mono">
          ...
          <UBadge
            v-if="orderDetail"
            size="lg"
            :color="statusColor(orderDetail.status_type)"
            variant="soft"
    ...
    <div class="mx-auto max-w-5xl">
      ...
      >
        ...
        <UButton variant="soft" color="primary" @click="$router.back()"
          >Quay lại</UButton
        >
      </div>
      ...
      <UContainer v-else class="py-5 space-y-3 pb-10">
        ...
        <UCard :ui="{ body: 'p-0' }">
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-1.5 text-xs text-muted">
                ...
                <span class="font-medium">{{
                  formatTime(orderDetail.date_of_destination)
                }}</span>
              </div>
            </div>
          </template>
          ...
          <div class="px-4 py-4 flex gap-3 items-start">
            <div class="flex flex-col items-center mt-0.5 shrink-0">
              ...
              <div
                class="w-px h-8 bg-linear-to-b from-blue-200 to-orange-200 my-1"
              />
              <div
                class="w-8 h-8 rounded-full bg-orange-50 flex items-center justify-center"
              >
            ...
            <div class="flex flex-col gap-5 flex-1 min-w-0">
              <div>
                ...
                <p class="text-sm font-semibold leading-snug">
                  {{ orderDetail.departure.address_1 }}
                </p>
                <p class="text-xs text-muted mt-0.5">
                  {{
                    [orderDetail.departure.district, orderDetail.departure.city]
                      .filter(Boolean)
                      .join(",")
                  }}
                </p>
              ...
              <div>
                <p
                  class="text-[10px] font-bold text-orange-500 uppercase tracking-wider mb-0.5"
                >
                  Điểm đến
                </p>
                <p class="text-sm font-semibold leading-snug">
                  {{ orderDetail.destination.address_1 }}
                </p>
                <p class="text-xs text-muted mt-0.5">
                  {{
                    [
                      orderDetail.destination.district,
                      orderDetail.destination.city,
                    ]
                      .filter(Boolean)
                      .join(", ")
                  }}
        ...
        <UCard v-if="orderDetail.partner">
          ...
          <div class="flex items-center gap-3">
            <div
              class="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center shrink-0 border-2 border-primary/20"
            >
          ...
          <template v-if="orderDetail.partner.transport" #footer>
            <div
              class="-mx-4 -my-3 px-4 py-3 bg-gradient-to-br from-slate-50 to-orange-50/30 rounded-b-md"
            >
              <div class="flex items-start gap-3">
                <div
                  class="w-10 h-10 rounded-lg bg-white flex items-center justify-center shrink-0 border border-default shadow-sm"
                >
                  <UIcon
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <UBadge
                      v-if="orderDetail.partner.transport.created_year"
                      color="neutral"
                      variant="outline"
                      size="xs"
                    >
                  ...
                  <div
                    class="mt-1.5 inline-flex items-center gap-1.5 px-2 py-1 rounded bg-white border border-default"
                  >
        ...
        <UCard>
          ...
          <div class="space-y-3 text-sm">
            <div
              v-if="orderDetail.created"
              class="flex justify-between items-center"
            >
              <span class="text-muted flex items-center gap-1.5">
                <UIcon name="i-lucide-plus-circle" class="size-3.5" />Đặt chuyến
                lúc
              </span>
              <span class="font-medium">{{
                formatTime(orderDetail.created)
              }}</span>
            </div>
            <div
              v-if="orderDetail.date_of_destination"
              class="flex justify-between items-center"
            >
              <span class="text-muted flex items-center gap-1.5">
                <UIcon name="i-lucide-calendar-clock" class="size-3.5" />Giờ
                khởi hành
              </span>
              <span class="font-medium">{{
                formatTime(orderDetail.date_of_destination)
            ...
            <div
              v-if="orderDetail.active_time"
              class="flex justify-between items-center"
            >
              <span class="text-muted flex items-center gap-1.5">
                <UIcon name="i-lucide-play-circle" class="size-3.5" />Hoạt động
                từ
              </span>
              <span class="font-medium">{{
                formatTime(orderDetail.active_time)
              }}</span>
            </div>
          </div>
        </UCard>
