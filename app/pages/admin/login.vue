<template>
  <ClientOnly>
    <div class="w-screen min-h-screen grid grid-cols-1 md:grid-cols-2 bg-slate-50">
      ...
      <div class="w-full min-h-screen flex flex-col justify-center items-center px-6 py-12">
        ...
        <p class="text-muted mt-1 text-sm text-center">
          Đăng nhập với tài khoản admin để quản lý nội dung
        </p>
...
const form = reactive<Schema>({ phone: '', password: '' })
...
async function onSubmit(_e: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    errorMsg.value =
      e?.data?.message ||
      e?.message ||
      'Không thể đăng nhập. Vui lòng kiểm tra lại số điện thoại và mật khẩu.'
  } finally {
    loading.value = false
  }
}
