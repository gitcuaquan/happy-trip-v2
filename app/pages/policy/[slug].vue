<template>
  <div class="min-h-screen bg-white dark:bg-slate-950 pb-16">
    ...
    <div class="mx-auto max-w-4xl px-4 sm:px-6 py-6 sm:py-8 space-y-8">
      <!-- Loading -->
      ...
      <!-- Error -->
      <div v-else-if="error || !article" class="flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-50 dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800">
      ...
      <article v-else class="space-y-8">
        ...
        <div v-if="otherPolicies?.length" class="pt-8 border-t border-slate-100 dark:border-slate-800 space-y-4">
          ...
          <div class="flex flex-wrap gap-2">
            <NuxtLink
              ...
              :key="item.id"
              :to="`/policy/${item.slug}`"
              class="px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 hover:bg-primary/10 hover:text-primary text-xs font-semibold text-slate-700 dark:text-slate-300 transition-colors"
            >
              {{ item.title }}
            </NuxtLink>
          </div>
