<template>
  <div class="relative w-full max-w-md mx-auto my-10 p-2">
    ...
    <div
      class="absolute right-2 -top-8 z-20 animate-bounce [animation-duration:2.5s]"
    >
      <div
        class="bg-white px-4 py-2 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-2"
      >
        <UIcon
          name="i-lucide-sparkles"
          class="size-5 text-yellow-500 animate-pulse [animation-duration:0.5s]"
        />
    ...
    >
      ...
      >
        ...
        <UFormField :ui="{ error: 'text-xs text-left' }">
          <template #label>
            <span
              class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1 block"
        ...
        <Transition name="fade">
          <div v-if="hasRouteData" class="space-y-2 pt-2">
            <p
              class="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5"
            >
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <div class="relative z-10">
                  <p
                    class="text-xs font-black text-slate-800 leading-tight mb-0.5 truncate"
                  >
                    {{ item.name }}
                  </p>
                  <p
                    class="text-[10px] text-slate-500 font-medium mb-2 truncate"
                  >
                    {{ item.description }}
                  </p>

                  <div
                    v-if="getPreview(item.id)"
        ...
        <p>
          ...
          <b>Không có thêm phí nào khác</b>
        </p>
      ...
      <template #footer>
        <div
          class="flex items-center justify-center gap-2 bg-slate-50 py-3 rounded-xl border border-slate-100"
        >
    ...
    <UModal v-model:open="isModalOpen" :ui="{ body: 'p-5' }">
      <template #title>
        <div class="flex items-center gap-2">
          <UIcon
            :name="
              otpSent ? 'i-lucide-message-square-dot' : 'i-lucide-user-round'
            "
...
const schema = z.object({
  ...
  date_of_destination: z
    .date()
    .nullable()
    .refine((val) => val !== null, { message: "Vui lòng chọn thời gian" }),
